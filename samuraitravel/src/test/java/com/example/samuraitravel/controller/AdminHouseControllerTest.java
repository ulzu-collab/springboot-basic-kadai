...
@SpringBootTest // アプリが実際に実行される時と同じ設定や依存環境、コンキテクトを起動する。
@AutoConfigureMockMvc // MVCの機能を模倣するMockMvcをインスタンス注入する。
@ActiveProfiles("test")
public class AdminHouseControllerTest {
	...
	public void 未ログインの場合は管理者用の民宿一覧ページからログインページにリダイレクトする() throws Exception {
		mockMvc.perform(get("/admin/houses")) // HTTPリクエストを送信する振る舞いをさせる。
				.andExpect(status().is3xxRedirection()) // 期待する内容（HTTPステータスコードが3で始まる）を指定する。
				.andExpect(redirectedUrl("http://localhost/login")); // リダイレクト先のURL。
	}

	@Test
	@WithUserDetails("taro.samurai@example.com") // ログインするユーザー名を渡してログインする振る舞いをさせる。
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿一覧ページが表示されずに403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses")) // URLを送信する。
				.andExpect(status().isForbidden()); // ステータスコードが403 Forbiddenである。
	}
	...
	public void 管理者としてログイン済みの場合は管理者用の民宿一覧ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/admin/houses"))
				.andExpect(status().isOk()) // ステータスコードが200 OKである。
				.andExpect(view().name("admin/houses/index")); // 表示されたビューが指定のされたものか。
	}
	...
	@Test
	@WithUserDetails("hanako.samurai@example.com")
	@Transactional // アノテーションでデータベースの変更をロールバックする。
	public void 管理者としてログイン済みの場合は民宿登録後に民宿一覧ページにリダイレクトする() throws Exception {
		...
		// multipartメソッドを使ってマルチパートのフォーム送信をシミュレートする。
		mockMvc.perform(MockMvcRequestBuilders.multipart("/admin/houses/create").file(imageFile)
				.with(csrf())
				.param("name", "テスト民宿名")
				.param("description", "テスト説明")
	...
	public void 管理者としてログイン済みの場合は民宿更新後に民宿詳細ページにリダイレクトする() throws Exception {
		...
		Optional<House> optionalHouse = houseService.findHouseById(1);
		assertThat(optionalHouse).isPresent(); // isPresent()メソッドでOptionalHouse型のオブジェクトが存在するかどうかを検証する。
		House house = optionalHouse.get();
		assertThat(house.getName()).isEqualTo("テスト民宿名");
		assertThat(house.getDescription()).isEqualTo("テスト説明");
		assertThat(house.getPrice()).isEqualTo(5000);
